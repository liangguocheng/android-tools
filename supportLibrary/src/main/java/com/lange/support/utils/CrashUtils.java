...
public final class CrashUtils {
    ...
    static {
        try {
            PackageInfo pi = SupportUtils.getApp()
                    .getPackageManager()
                    .getPackageInfo(SupportUtils.getApp().getPackageName(), 0);
            if (pi != null) {
    ...
    public static void init(final String crashDirPath, final OnCrashListener onCrashListener) {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                && SupportUtils.getApp().getExternalCacheDir() != null)
            defaultDir = SupportUtils.getApp().getExternalCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        else {
            defaultDir = SupportUtils.getApp().getCacheDir() + FILE_SEP + "crash" + FILE_SEP;
        }
